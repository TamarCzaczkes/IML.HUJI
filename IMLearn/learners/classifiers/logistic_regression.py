...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        m = X.shape[1]
        self.coefs_ = np.random.randn(m) / np.sqrt(m)

        if self.penalty_ == "none":
            module = LogisticModule(self.coefs_)
        elif self.penalty_ == "l1":
            module = RegularizedModule(LogisticModule(), L1(), self.lam_,
                                       self.coefs_, self.include_intercept_)
        elif self.penalty_ == "l2":
            module = RegularizedModule(LogisticModule(), L2(), self.lam_,
                                       self.coefs_, self.include_intercept_)
