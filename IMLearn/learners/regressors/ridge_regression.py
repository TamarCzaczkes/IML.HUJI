...
class RidgeRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        # X_lam = np.concatenate((X, np.diag(np.sqrt(self.lam_) * np.ones(X.shape[1]))))

        X_lam = np.concatenate((X, np.eye(X.shape[1]) * np.sqrt(self.lam_)))
        y_lam = np.concatenate((y, np.zeros(X.shape[1])))
        self.coefs_ = pinv(X_lam) @ y_lam
        # self.coefs_ = np.linalg.inv(X_lam.T @ X_lam) @ X_lam.T @ y_lam  # another way, without pinv
