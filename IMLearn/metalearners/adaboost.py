...
class AdaBoost(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        for i in range(self.iterations_):
            # create, fit and predict base learner:
            model = self.wl_()
            model.fit(X, y * self.D_)
            self.models_.append(model)
            model_pred = model.predict(X)
            # calculate:
            epsilon = np.sum(self.D_[y != model_pred])
            self.weights_[i] = 0.5 * np.log((1 / epsilon) - 1)  # update weights
            self.D_ *= np.exp(-y * self.weights_[i] * model_pred)  # Update sample weights
    ...
    def partial_predict(self, X: np.ndarray, T: int) -> np.ndarray:
        ...
        for i in range(T):
            responses += self.models_[i].predict(X) * self.weights_[i]
